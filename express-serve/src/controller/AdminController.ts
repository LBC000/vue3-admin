...
import { MenuTree } from '../entity/MenuTree';
...
export class AdminController {
  ...
  private menuTreeRepository = AppDataSource.getRepository(MenuTree);
  ...
  // 新增或编辑菜单 -系统管理
  async addOrEditMenu(request: Request, response: Response, next: NextFunction) {
    /**
     * {
          "type":"0",
          "menuName":"系统设置",
  ...
  async getDeptList(request: Request, response: Response, next: NextFunction) {
    try {
      delete request.query._t;

      let opt: any = {};

      console.log(request.query, '参数1');
      if (request.query.status !== undefined) {
        opt.status = request.query.status;
      }
      if (request.query.deptName) {
        opt.deptName = Like(`%${request.query.deptName}%`);
      }
  ...
  // 获取角色列表
  async getAllRoleList(request: Request, response: Response, next: NextFunction) {
    return resFormatSuccess({ data: dataRoleList });
  }

  // 获取角色列表分页
  async getRoleListByPage(request: Request, response: Response, next: NextFunction) {
    return resFormatSuccess({ data: dataRoleListPage });
  }

  // 获取账号列表
  async getAccountList(request: Request, response: Response, next: NextFunction) {
